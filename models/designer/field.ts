/*
	Evolutility Model for Object's fields
	https://github.com/evoluteur/evolutility-models
	(c) 2026 Olivier Giulieri
*/

import { FieldType, type Model } from "../../scripts/types.js";

const fieldTypes = [
  { id: 1, text: "Text", icon: "designer/ft-txt.png" },
  { id: 2, text: "Text multiline", icon: "designer/ft-txtml.png" },
  { id: 3, text: "Boolean", icon: "designer/ft-bool.png" },
  { id: 4, text: "Decimal", icon: "designer/ft-dec.png" },
  { id: 5, text: "Money", icon: "designer/ft-money.png" },
  { id: 6, text: "Integer", icon: "designer/ft-int.png" },
  { id: 7, text: "Date", icon: "designer/ft-date.png" },
  { id: 8, text: "Time", icon: "designer/ft-time.png" },
  { id: 9, text: "Date-time", icon: "designer/ft-datetime.png" },
  { id: 10, text: "Image", icon: "designer/ft-img.png" },
  { id: 11, text: "Dropdown", icon: "designer/ft-lov.png" },
  { id: 12, text: "email", icon: "designer/ft-email.png" },
  { id: 13, text: "Link", icon: "designer/ft-url.png" },
  { id: 14, text: "Multiselect (list)", icon: "designer/ft-list.png" },
  { id: 15, text: "JSON", icon: "designer/ft-json.png" },
];

export const field = {
  id: "field",
  world: "designer",
  title: "Fields",
  schema: "evolutility",
  icon: "/designer/field.png",
  name: "field",
  namePlural: "fields",
  titleField: "label",
  table: "evol_field",
  position: 20,
  fields: [
    {
      id: "label",
      label: "Label",
      type: FieldType.text,
      help: "Field title in forms and tables",
      maxLength: 100,
      required: true,
      inMany: true,
      width: 62,
      column: "label",
      inSearch: true,
    },
    {
      id: "type",
      type: FieldType.lov,
      label: "Type",
      list: fieldTypes,
      defaultValue: 1,
      help: "Type of field: UI component types rather than data type.",
      width: 38,
      required: true,
      column: "type_id",
      lovTable: "evol_field_type",
      lovColumn: "name",
      lovIcon: true,
      inMany: true,
    },
    {
      id: "column",
      column: "dbcolumn",
      label: "Column",
      help: "Database column name",
      required: true,
      type: FieldType.text,
      maxLength: 63,
      width: 22,
      inSearch: true,
    },
    {
      id: "cf",
      column: "iscustom",
      label: "Custom field",
      help: "Custom field values do not have a column in the DB table, their values are stored together in one JSON field.",
      required: true,
      defaultValue: false,
      type: FieldType.boolean,
      width: 20,
      inSearch: true,
    },
    {
      id: "fid",
      label: "Field ID",
      type: FieldType.text,
      help: "Field ID (not visible to the users) are used to identify the field in API and routing.",
      required: true,
      inMany: true,
      width: 62,
      column: "fid",
      inSearch: true,
    },
    {
      id: "object",
      label: "Object",
      type: FieldType.lov,
      object: "object",
      required: true,
      inMany: true,
      noCharts: true,
      width: 38,
      column: "object_id",
      lovTable: "evol_object",
      lovColumn: "title",
      deleteTrigger: true,
    },
    {
      id: "lovTable",
      column: "lovtable",
      label: "LOV Table",
      help: "Lookup table",
      type: FieldType.text,
      maxLength: 63,
      width: 32,
    },
    {
      id: "lovColumn",
      column: "lovcolumn",
      label: "LOV column",
      type: FieldType.text,
      help: "Column fom Lookup table to display",
      maxLength: 63,
      width: 38,
    },
    {
      id: "lovIcon",
      column: "lovicon",
      label: "LOV Icon",
      type: FieldType.text,
      help: "Column fom Lookup table to display",
      maxLength: 63,
      width: 38,
    },
    {
      id: "dependantField",
      label: "Dependant Field ID",
      type: FieldType.text,
      column: "dep_field_id",
      lovTable: "evol_field",
      help: 'ID of a dependant field. Used when selecting in one list changes the content of dependant list (among 2 fields of type "lov").',
      maxLength: 50,
      inMany: true,
      width: 62,
      inSearch: true,
    },
    {
      id: "inMany",
      column: "inmany",
      label: "In List",
      help: "Field is displayed in summary lists (List and Cards views in the UI)",
      type: FieldType.boolean,
      inMany: true,
      width: 30,
    },
    {
      id: "inSearch",
      column: "insearch",
      label: "In Search",
      help: "Field is used for text searches.",
      type: FieldType.boolean,
      inMany: true,
      width: 30,
    },
    {
      id: "pii",
      column: "pii",
      label: "PII",
      help: "Flag for personally identifiable information.",
      type: FieldType.boolean,
      inMany: true,
      width: 30,
      img: "lock.png",
    },
    {
      id: "noFilters",
      column: "noFilters",
      label: "In Filter",
      help: "Excludes the field from filters' conditions.",
      type: FieldType.boolean,
      inMany: true,
      width: 50,
    },
    {
      id: "position",
      column: "position",
      label: "Position",
      help: "Order of the field",
      type: FieldType.integer,
      maxLength: 3,
      width: 50,
      noCharts: true,
    },
    {
      id: "width",
      column: "width",
      label: "Width",
      defaultValue: 100,
      help: "Relative width of the field (in percentage)",
      type: FieldType.integer,
      format: "0 '%'",
      maxLength: 3,
      width: 50,
    },
    {
      id: "height",
      column: "height",
      label: "Height",
      help: 'Height in number of lines (for "Textmultiline" fields)',
      type: FieldType.integer,
      maxLength: 3,
      defaultValue: 1,
      max: 30,
      width: 50,
    },
    {
      id: "css",
      column: "css",
      label: "CSS",
      help: "Stylesheet class name for the field for the edit view.",
      type: FieldType.text,
      maxLength: 20,
      width: 50,
    },
    {
      id: "format",
      column: "format",
      label: "Format",
      type: FieldType.text,
      help: 'example "$ 0.00"',
      maxLength: 30,
      width: 50,
    },
    {
      id: "labelShort",
      label: "Short label",
      type: FieldType.text,
      help: "Optional shorter Field title to display in list header",
      width: 100,
      column: "labelshort",
    },
    {
      id: "required",
      column: "required",
      label: "Required",
      help: "Mandatory field",
      type: FieldType.boolean,
      inMany: true,
      width: 50,
      img: "checkr.gif",
    },
    {
      id: "readOnly",
      column: "readonly",
      label: "Read only",
      defaultValue: false,
      help: "Users can view this field value but cannot modify it",
      type: FieldType.boolean,
      width: 50,
      img: "checkr.gif",
    },
    {
      id: "minLength",
      column: "minlength",
      label: "Min. length",
      help: "Minimum number of characters required",
      type: FieldType.integer,
      width: 50,
      noCharts: true,
    },
    {
      id: "maxLength",
      column: "maxlength",
      label: "Max. length",
      help: "Maximum number of characters allowed",
      type: FieldType.integer,
      maxLength: 7,
      width: 50,
      noCharts: true,
    },
    {
      id: "minValue",
      label: "Min. value",
      labelShort: "Min.",
      help: "Minimum value allowed for the field",
      type: FieldType.integer,
      maxLength: 4,
      width: 50,
      noCharts: true,
      column: "minvalue",
    },
    {
      id: "maxValue",
      label: "Max. value",
      labelShort: "Max.",
      help: "Maximum value allowed for the field",
      type: FieldType.integer,
      maxLength: 4,
      width: 50,
      noCharts: true,
      column: "maxvalue",
    },
    {
      id: "regExp",
      column: "regexp",
      label: "Regular Expression",
      labelShort: "RegExp",
      type: FieldType.text,
      maxLength: 100,
      width: 50,
      help: "Regular expression used to validate the field value.",
    },
    {
      id: "noCharts",
      column: "nocharts",
      label: "Exclude from Charts",
      type: FieldType.boolean,
      width: 50,
      help: "Determines if the field's charts appears in the dashboard.",
    },
    {
      id: "noStats",
      column: "nostats",
      label: "Exclude from Stats",
      type: FieldType.boolean,
      width: 50,
      help: "Determines if the field's stats appears in the Stats view.",
    },
    {
      id: "hideIfEmpty",
      column: "hideifempty",
      label: "Hide when empty",
      type: FieldType.boolean,
      width: 50,
      help: "Hides the field in Browse view.",
    },
    {
      id: "help",
      column: "help",
      label: "Help",
      help: "Help on the field in Edit view",
      type: FieldType.textmultiline,
      maxLength: 500,
      width: 100,
      height: 4,
      inSearch: true,
    },
    {
      id: "description",
      column: "description",
      label: "Description",
      type: FieldType.textmultiline,
      maxLength: 500,
      width: 100,
      height: 6,
      inSearch: true,
    },
    {
      id: "defaultValue",
      column: "defaultvalue",
      label: "Default Value",
      type: FieldType.json,
      width: 50,
    },
    {
      id: "unique",
      column: "unique",
      label: "Unique",
      help: "Field value is unique.",
      type: FieldType.boolean,
      inMany: true,
      width: 50,
    },
  ],
  groups: [
    {
      type: "panel",
      label: "Definition",
      width: 62,
      fields: [
        "label",
        "type",
        "object",
        "column",
        "fid",
        "inMany",
        "inSearch",
        "inFilter",
        "pii",
        "custom",
        "lovTable",
        "lovColumn",
        "lovIcon",
      ],
    },
    {
      type: "panel",
      label: "Layout",
      width: 38,
      fields: [
        "hideifempty",
        "position",
        "width",
        "height",
        "css",
        "format",
        "labelShort",
      ],
    },
    {
      type: "pValidation",
      label: "Validation",
      width: 62,
      fields: [
        "defaultValue",
        "required",
        "readOnly",
        "dependantField",
        "minValue",
        "maxValue",
        "minLength",
        "maxLength",
        "regExp",
        "unique",
        "noCharts",
        "noStats",
      ],
    },
    {
      id: "pHelp",
      type: "panel",
      label: "Field Help",
      width: 38,
      fields: ["help", "description"],
    },
  ],
} satisfies Model;
